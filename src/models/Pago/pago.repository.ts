...
export class PagoRepository {
  ...
  /**
   * Paginated version of findAllWithFilters that paginates by distinct obras.
   * Returns all pagos belonging to the obras on the current page.
   */
  async findAllWithFiltersPaginated(
    ...
    // Get distinct cod_obra values matching filters
    const distinctObras = await this.prisma.pago.findMany({
      where: whereConditions,
      distinct: ['cod_obra'],
      select: { cod_obra: true },
      orderBy: { fecha_pago: 'desc' },
    })

    const totalObras = distinctObras.length
    const skip = (pagination.page - 1) * pagination.pageSize
    const paginatedObraCodes = distinctObras
      .slice(skip, skip + pagination.pageSize)
      .map(o => o.cod_obra)

    // Fetch all pagos for the paginated obras
    const pagos = await this.prisma.pago.findMany({
      where: {
        ...whereConditions,
        cod_obra: { in: paginatedObraCodes },
      },
      orderBy: { fecha_pago: 'desc' },
      include: {
        obra: {
    ...
    return { data: pagos, total: totalObras }
  }
